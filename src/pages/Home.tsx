...
export default function Home() {
  ...
  return (
    <motion.div 
      ...
      className="space-y-12 text-ink"
    >
      {/* Discovery Hero */}
      <section className="relative h-[224px] rounded-[2.5rem] overflow-hidden flex flex-col items-center justify-center text-center px-6 border border-hairline">
        <SmartImage
            ...
            <div className="relative flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white/80 backdrop-blur-md border border-hairline rounded-2xl py-4 pl-12 pr-6 text-ink font-bold shadow-2xl focus:ring-2 focus:ring-orange-500 transition-all placeholder:font-normal"
                />
                ...
                <AnimatePresence>
                  {showDropdown && searchQuery.trim() && (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full left-0 right-0 mt-3 bg-white border border-hairline shadow-2xl overflow-hidden z-50 text-left"
                      style={{ borderRadius: '1.2rem' }}
                    >
                      <div className="max-h-[50vh] overflow-y-auto">
                        <div className="px-5 py-4 border-b border-hairline bg-tint">
                          <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest flex items-center gap-2"><Users size={12}/> Top Travelers</h3>
                          {hubUsers.length === 0 ? <p className="text-xs text-muted font-medium mt-2">No users matching query.</p> : (
                            <div className="grid grid-cols-1 gap-1.5 mt-2">
                              {hubUsers.map(u => (
                                <div 
                                  key={u.id}
                                  onClick={() => navigate(`/user/${u.username}`)}
                                  className="flex items-center gap-3 p-2.5 bg-white border border-hairline hover:border-orange-500 cursor-pointer group transition-all rounded-xl"
                                >
                                  <div className="w-8 h-8 bg-cream rounded-full overflow-hidden shrink-0 border border-hairline">
                                    {u.avatar_url ? <img src={u.avatar_url} alt={u.username} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-muted"><Users size={12}/></div>}
                                  </div>
                                  <div className="flex flex-col">
                                    <span className="font-bold text-ink text-sm leading-tight group-hover:text-orange-500 transition-colors truncate">{u.full_name || u.username || 'Anonymous'}</span>
                        ...
                        <div className="px-5 py-4 bg-white">
                          <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest flex items-center gap-2"><MapPinIcon size={12}/> Top Itineraries</h3>
                          {topTrips.length === 0 ? <p className="text-xs text-muted font-medium mt-2">No trip locations match.</p> : (
                            <div className="grid grid-cols-1 gap-1.5 mt-2">
                              {topTrips.map(p => (
                                <div 
                                  key={p.id}
                                  onClick={() => { setSearchQuery(p.location); navigate('/search'); setShowDropdown(false); }}
                                  className="flex items-center gap-3 p-2.5 bg-tint border border-hairline hover:border-ink hover:bg-white cursor-pointer group transition-all rounded-xl"
                                >
                                  ...
                                  <div className="flex flex-col flex-1 min-w-0">
                                     <span className="font-bold text-ink text-sm leading-tight group-hover:text-orange-500 transition-colors truncate">{p.caption.split('#')[0]}</span>
                                     <span className="text-body text-[9px] uppercase font-bold flex items-center gap-1 truncate"><MapPinIcon size={10} className="text-orange-500"/> {p.location}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
                      ...
                      <div className="p-3 bg-ink text-white text-center font-bold text-xs uppercase tracking-widest hover:bg-black transition-colors cursor-pointer" onClick={(e) => { e.preventDefault(); navigate('/search'); setShowDropdown(false); }}>
              ...
              <button 
                ...
                className={`p-4 rounded-2xl backdrop-blur-md border transition-all ${
                  showFilters 
                    ...
                    : 'bg-white/10 border-hairline text-ink hover:bg-white/20'
                }`}
              >
                <Filter size={24} />
              </button>
            ...
            <AnimatePresence>
              {showFilters && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -20 }}
                  className="bg-white/95 backdrop-blur-md border border-hairline rounded-3xl p-4 shadow-2xl"
                >
                  <FilterBar />
                </motion.div>
              )}
      ...
      <div className="flex items-center justify-center -mt-8">
        <div className="bg-white/50 backdrop-blur-md border border-hairline p-1.5 rounded-2xl flex gap-1">
          {[
        ...
        {activeTab === 'Feed' ? (
          ...
          >
            ...
            <div className="flex justify-center mb-8">
              <div className="bg-white border border-hairline p-1 rounded-full flex gap-1 shadow-sm">
                <button
                  onClick={() => setFeedMode('Discover')}
                  className={`flex items-center gap-2 px-6 py-2 rounded-full font-bold text-sm transition-all ${
                    feedMode === 'Discover' ? 'bg-ink text-white' : 'text-body hover:text-ink'
                  }`}
                >
                  Discover
                </button>
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2 rounded-full font-bold text-sm transition-all ${
                    feedMode === 'Following' ? 'bg-ink text-white' : 'text-body hover:text-ink'
                  }`}
                >
                  Following
                </button>
            ...
            {/* Create Post Prompt */}
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-hairline flex items-center gap-4">
              <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 border-2 border-orange-500/20">
                <img src={activeProfile?.avatar_url || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&h=150"} alt={activeProfile?.username || "Current User"} className="w-full h-full object-cover bg-cream" />
              </div>
              <button 
                onClick={() => setIsPublishModalOpen(true)}
                className="flex-1 text-left bg-tint hover:bg-cream transition-colors py-3.5 px-6 rounded-full text-body font-medium border border-hairline"
              >
            ...
            <div className="bg-transport-tint border border-transport/20 p-4 rounded-3xl flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-transport text-white rounded-full">
                  <Navigation size={18} />
                </div>
                <div>
                  <h4 className="font-bold text-ink text-sm md:text-base">Local Discovery</h4>
                  {userLocation ? (
                    <p className="text-transport text-xs md:text-sm font-medium">Sorting by real-time geographical proximity</p>
                  ) : (
                    <p className="text-body text-xs md:text-sm">Turn on live location for precise local recommendations.</p>
                  )}
                </div>
              </div>
              {!userLocation && (
                <button 
                  ...
                  className="bg-orange-500 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-md shadow-orange-500/20 hover:bg-orange-600 transition-colors"
                >
                  Enable
                </button>
              )}
            ...
            {isDbLoading ? (
              ...
              feedMode === 'Following' ? (
                <div className="bg-white rounded-3xl p-12 text-center border border-hairline shadow-sm flex flex-col items-center">
                  <div className="w-16 h-16 bg-transport-tint text-transport rounded-full flex items-center justify-center mb-4">
                    <Users size={32} />
                  </div>
                  <h3 className="text-xl font-bold text-ink mb-2">Build Your Network</h3>
                ...
                <div className="bg-white rounded-[2rem] p-16 text-center border border-hairline shadow-sm flex flex-col items-center">
                  ...
                  <h3 className="text-2xl font-bold text-ink mb-4">No trips published yet</h3>
              ...
              homePosts.map((post) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className={`bg-white rounded-3xl overflow-hidden shadow-sm border ${userInterestTags.some(tag => post.location.includes(tag)) ? 'border-orange-500/50 shadow-orange-500/10' : 'border-hairline'}`}
                >
                  {/* Post Header */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center flex-wrap gap-2">
                          ...
                          <button 
                            ...
                            className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border transition-colors ${
                              followedUsers.includes(post.userId)
                                ? 'bg-tint text-body border-hairline hover:border-rose-200 hover:text-rose-500 hover:bg-rose-50' // Following (gives unfollow context on hover)
                                : requestedUsers.includes(post.userId)
                                ? 'bg-cream text-body border-hairline hover:bg-zinc-200' // Requested
                                : 'bg-orange-500 text-white border-orange-500 hover:bg-orange-600' // Not Following (Public or Private)
                            }`}
                          >
                            {followedUsers.includes(post.userId) 
                              ? 'Following' 
                          ...
                          <span className="text-muted font-normal ml-1">—</span> 
                          <span className="text-orange-500">{post.location}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <p className="text-xs text-muted font-medium">2 hours ago</p>
                  ...
                  <div className="p-6 space-y-4">
                    <div className="flex items-center gap-6">
                      <button 
                        ...
                        className={`flex items-center gap-2 transition-colors group ${userLikedPosts.includes(post.id) ? 'text-rose-500' : 'text-ink hover:text-rose-500'}`}
                      >
                      ...
                      <button className="flex items-center gap-2 text-ink hover:text-orange-500 transition-colors group">
                    ...
                    <div className="space-y-2">
                      <p className="text-zinc-600 leading-relaxed">
                        <span className="font-bold text-ink mr-2">{post.user}</span>
                        {post.caption}
                      </p>
                        <div className="flex flex-wrap gap-2 pt-2">
                          {post.activities.map(activity => (
                            <span key={activity} className="px-3 py-1 rounded-full bg-tint text-[10px] font-bold text-body uppercase tracking-widest border border-hairline">
                              {activity}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-hairline mt-4">
                        <button 
                          onClick={() => {
                            addUserInterest(post.location.split(',')[0]);
                            navigate(`/trip/${post.id}`);
                          }}
                          className="text-body text-sm font-bold hover:text-ink transition-colors"
                        >
                          View Itinerary
                        </button>
