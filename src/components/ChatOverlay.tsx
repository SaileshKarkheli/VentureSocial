  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed bottom-6 right-6 w-80 sm:w-96 bg-white border-2 border-ink shadow-2xl z-50 flex flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white border-b border-hairline">
          {messages.map(m => (
            <div key={m.id} className={`flex flex-col ${m.sender === 'me' ? 'items-end' : 'items-start'}`}>
              <div className={`px-4 py-2 max-w-[85%] text-sm font-bold border-2 ${
                m.sender === 'me' 
                  ? 'bg-ink border-ink text-white text-right' 
                  : 'bg-white border-hairline text-ink text-left'
        ...
        <div className="p-3 bg-tint flex items-center gap-2 border-t border-hairline">
           <input
             type="text"
             value={input}
             ...
             placeholder="Message..."
             className="flex-1 bg-white border border-zinc-300 px-3 py-2 text-sm focus:border-orange-500 focus:ring-0 focus:outline-none placeholder:text-muted font-bold"
             style={{ borderRadius: '0' }}
           />
