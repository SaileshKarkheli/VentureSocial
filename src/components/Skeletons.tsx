...
export function FeedSkeleton() {
  return (
    <div className="space-y-6 w-full">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white border border-zinc-100 rounded-[2rem] p-4 flex flex-col gap-4 animate-pulse shadow-sm">
          {/* Header */}
...
export function TripGridSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {[1, 2, 3, 4, 5, 6].map((i) => (
        <div key={i} className="aspect-[4/5] bg-white border border-zinc-100 p-2 rounded-[2rem] flex flex-col gap-2 animate-pulse shadow-sm">
