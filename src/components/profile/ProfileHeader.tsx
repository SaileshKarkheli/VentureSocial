  ...
  return (
    <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden mb-6">
      ...
      <div className="px-6 md:px-10 pb-8 relative">
        {/* Avatar */}
        <div className="absolute -top-16 border-4 border-white rounded-3xl bg-zinc-100 w-32 h-32 overflow-hidden shadow-lg">
          {profile?.avatar_url ? (
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between mt-20 md:mt-4 gap-4">
          <div className="md:ml-36">
            <h1 className="text-3xl font-display font-bold text-[#0A192F]">{profile?.full_name || 'Anonymous User'}</h1>
          ...
          <div className="flex-shrink-0">
            {isOwner ? (
              <button 
                onClick={onEdit}
                className="flex items-center gap-2 px-6 py-3 bg-zinc-100 text-[#0A192F] font-bold rounded-2xl hover:bg-zinc-200 transition-all border border-zinc-200"
              >
                <Edit3 size={18} />
                Edit Profile
              </button>
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-8 py-3 font-bold rounded-2xl transition-all shadow-xl ${
                  isFollowing 
                    ? 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200 border border-zinc-200 shadow-none' 
                    : 'bg-[#0A192F] text-white shadow-blue-500/20 hover:bg-black'
                }`}
              >
